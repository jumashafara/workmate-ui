              ...
              >
                <Popup maxWidth={400} maxHeight={500}>
                  <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
                    {/* Header */}
                    <div style={{ marginBottom: '12px', borderBottom: '2px solid #ccc', paddingBottom: '8px' }}>
                      <strong style={{ fontSize: '16px', color: getMarkerColor(h.prediction) }}>
                        {h.prediction === 0 ? '🔴 Not Likely to hit the target' : '🟢 Likely to hit the target'}
                      </strong>
                    </div>

                    {/* Basic Information */}
                    <div style={{ marginBottom: '12px' }}>
                      <h4 style={{ margin: '0 0 8px 0', color: '#333' }}>Basic Information</h4>
                      ...
                      {h.cluster && <><strong>Cluster:</strong> {h.cluster}<br /></>}
                      <strong>Coordinates:</strong> {h.latitude.toFixed(6)}, {h.longitude.toFixed(6)}<br />
                      {h.altitude && <><strong>Altitude:</strong> {h.altitude.toFixed(1)}m<br /></>}
                    </div>
                    ...
                    {(h.cohort || h.cycle || h.evaluation_month) && (
                      <div style={{ marginBottom: '12px' }}>
                        <h4 style={{ margin: '0 0 8px 0', color: '#333' }}>Evaluation Details</h4>
                        {h.cohort && <><strong>Cohort:</strong> {h.cohort}<br /></>}
                        {h.cycle && <><strong>Cycle:</strong> {h.cycle}<br /></>}
                        {h.evaluation_month && <><strong>Evaluation Month:</strong> {h.evaluation_month}<br /></>}
                        {h.standard_evaluation !== undefined && <><strong>Standard Evaluation:</strong> {formatBoolean(h.standard_evaluation)}<br /></>}
                        {h.checkin_evaluation !== undefined && <><strong>Checkin Evaluation:</strong> {formatBoolean(h.checkin_evaluation)}<br /></>}
                      </div>
                    )}

                    {/* Household Demographics */}
                    <div style={{ marginBottom: '12px' }}>
                      <h4 style={{ margin: '0 0 8px 0', color: '#333' }}>Demographics</h4>
                      {h.tot_hhmembers && <><strong>Total Members:</strong> {h.tot_hhmembers}<br /></>}
                      {h.hhh_sex !== undefined && <><strong>Head Gender:</strong> {h.hhh_sex === 1 ? 'Female' : 'Male'}<br /></>}
                      {h.education_level_encoded !== undefined && <><strong>Education Level:</strong> {h.education_level_encoded}<br /></>}
                    </div>

                    {/* Agriculture & Livestock */}
                    {(h.Land_size_for_Crop_Agriculture_Acres || h.farm_implements_owned) && (
                      <div style={{ marginBottom: '12px' }}>
                        <h4 style={{ margin: '0 0 8px 0', color: '#333' }}>Agriculture & Livestock</h4>
                        {h.Land_size_for_Crop_Agriculture_Acres !== undefined && <><strong>Farm Size:</strong> {h.Land_size_for_Crop_Agriculture_Acres} acres<br /></>}
                        {h.farm_implements_owned && <><strong>Farm Implements:</strong> {h.farm_implements_owned}<br /></>}
                      </div>
                    )}
                    ...
                    {(h.business_participation || h.vsla_participation) && (
                      <div style={{ marginBottom: '12px' }}>
                        <h4 style={{ margin: '0 0 8px 0', color: '#333' }}>Economic Activities</h4>
                        {h.business_participation !== undefined && <><strong>Business Participation:</strong> {formatBoolean(h.business_participation)}<br /></>}
                        {h.vsla_participation !== undefined && <><strong>VSLA Participation:</strong> {formatBoolean(h.vsla_participation)}<br /></>}
                      </div>
                    )}

                    {/* Health Access */}
                    {h.Distance_travelled_one_way_OPD_treatment !== undefined && (
                      <div style={{ marginBottom: '12px' }}>
                        <h4 style={{ margin: '0 0 8px 0', color: '#333' }}>Health Access</h4>
                        <strong>Distance to OPD:</strong> {h.Distance_travelled_one_way_OPD_treatment}<br />
                      </div>
                    )}

                    {/* Other Features */}
                    <div>
                      <h4 style={{ margin: '0 0 8px 0', color: '#333' }}>Other Features</h4>
                      {h.composts_num !== undefined && <><strong>Composts:</strong> {h.composts_num}<br /></>}
                      {h.hh_produce_lq_manure !== undefined && <><strong>Produces Liquid Manure:</strong> {formatBoolean(h.hh_produce_lq_manure)}<br /></>}
                      {h.hh_produce_organics !== undefined && <><strong>Produces Organics:</strong> {formatBoolean(h.hh_produce_organics)}<br /></>}
